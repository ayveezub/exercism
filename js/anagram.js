...
export const findAnagrams = (targetWord, candidateWords) => {
  return candidateWords.filter(candidateWord => {
    if (candidateWord.length !== targetWord.length) return false
    if (candidateWord.toLowerCase() === targetWord.toLowerCase()) return false
    return candidateWord.toLowerCase().split('').sort().join('') === targetWord.toLowerCase().split('').sort().join('')
  })
}
