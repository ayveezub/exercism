...
export const compute = (firstStrand, secondStrand) => {
  if (firstStrand.length !== secondStrand.length) throw new Error('strands must be of equal length')

  return firstStrand.split('').reduce((acc, currentLetter, ind) => (
    currentLetter !== secondStrand.charAt(ind) ? ++acc : acc
  ), 0)
}
