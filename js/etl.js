...
The goal: We're going to extract some Scrabble scores from a legacy system.
...
Notes: A final note about scoring,
Scrabble is played around the world in a variety of languages, each with its own unique scoring table.
...
export const transform = legacy => {
  ...
  const legacyLetters = Object.values(legacy).flat()

  return Object.fromEntries(legacyLetters.map(letter => {
    const newLetter = letter.toLowerCase()
    
    return [newLetter, newScrabbleSystem[newLetter]]
  }))
}
